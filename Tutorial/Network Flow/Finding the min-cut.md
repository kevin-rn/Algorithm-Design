...
public class UTest {
  ...
  public void example() {
    ...
    assertEquals("Should take only project 1 as that has a net profit: Size", 2, answer.size());
  ...
  public void example02() {
    ...
    Set<Integer> answer = TheGreedyProjectManager.outputSelection(n, b, c, dep);
    assertEquals("Project 1 (profit 6) now requires 2 (loss 2), so we take both", 3, answer.size());
...
class TheGreedyProjectManager {
  ...
  private static Set<Integer> bfs(Graph g) {
    ...
    res.add(g.getSource().getId());
    while (!q.isEmpty()) {
      Node cur = q.poll();
      for (Edge e : cur.getEdges()) {
        if (e.getFlow() < e.getCapacity() && !res.contains(e.to.getId())) {
          q.add(e.to);
          res.add(e.to.getId());
        }
      }
